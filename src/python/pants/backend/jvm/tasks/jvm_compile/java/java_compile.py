...
class JavaCompile(JvmCompile):
  ...
  def compute_classpath_elements_by_class(self, classpath):
    """Since jmake doesn't include the full path to classes in the analysis file
    we need to also consider classes generated by other build steps, such as scala compilation.
    Otherwise jmake_analysis_parser assumes the class was generated by the local compiler (java only).
    """
    super(JavaCompile, self).compute_classpath_elements_by_class(classpath)

    for path in classpath:
      if os.path.isdir(path):
        for dirpath, _, filenames in os.walk(path, followlinks=True):
          for f in filter(lambda x: x.endswith('.class'), filenames):
            cls = os.path.relpath(os.path.join(dirpath, f), path)
            if not cls in self._class_to_path:
              self._class_to_path[cls] = os.path.join(path, cls)

    return self._class_to_path
